      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-sm">
        <div className="bg-card rounded-lg border border-border p-8 shadow-card">
          ...
          <div className="text-center mb-6">
            <h2 className="font-display text-xl font-bold text-foreground mb-2">Session Recorder</h2>
            <p className="text-sm text-muted-foreground">Enter the password to record game sessions</p>
          </div>

          {/* Password Form */}
          <form onSubmit={handleSubmit} className="space-y-4">
            <motion.div animate={isShaking ? { x: [-10, 10, -10, 10, 0] } : { x: 0 }} transition={{ duration: 0.4 }}>
