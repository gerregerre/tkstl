...
import { Plus, X, Check, ChevronDown, Calendar, Users, Trophy, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { usePlayers } from '@/hooks/usePlayers';
import { useSessionSignups } from '@/hooks/useSessionSignups';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function SessionSignup({ sessionDate: _sessionDate }: SessionSignupProps) {
  ...
  const isLoading = playersLoading || signupsLoading;

  // Get available players (not already selected) - sorted alphabetically
  const availablePlayers = useMemo(() => {
    return players
      .filter(p => !signedUpPlayerIds.includes(p.id))
      .sort((a, b) => a.name.localeCompare(b.name));
  }, [players, signedUpPlayerIds]);

  const handleSelectPlayer = async (slotIndex: number, player: { id: string; name: string }) => {
    setOpenPopover(null);
    await addPlayer(slotIndex + 1, player); // slot_number is 1-indexed in DB
  };

  const handleRemovePlayer = async (slotIndex: number) => {
    await removePlayer(slotIndex + 1); // slot_number is 1-indexed in DB
  };
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[0, 1, 2, 3].map((index) => (
              <div 
                key={index}
                className="flex items-center gap-3 px-3 py-3 rounded-md border-2 border-dashed border-border/40 animate-pulse"
              >
                <div className="w-9 h-9 rounded-md bg-muted/50" />
                  ...
                  <div className={cn(
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100 disabled:opacity-50"
