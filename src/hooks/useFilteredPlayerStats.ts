...
export function useFilteredPlayerStats(filter: GameTypeFilter) {
  ...
  const playerStats = useMemo((): FilteredPlayerStats[] => {
    const statsMap = new Map<string, { totalPoints: number; gamesPlayed: number }>();
    ...
    sessionGames.forEach((game) => {
      ...
      [game.team_a_player1, game.team_a_player2].forEach((player) => {
        const existing = statsMap.get(player) || { totalPoints: 0, gamesPlayed: 0 };
        statsMap.set(player, {
  ...
  const teamStats = useMemo((): FilteredTeamStats[] => {
    const statsMap = new Map<string, { player1: string; player2: string; totalPoints: number; gamesPlayed: number }>();
