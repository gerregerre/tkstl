          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {recentGames.length < 5 && (
                <span className="text-xs text-muted-foreground ml-1">({5 - recentGames.length} more to show)</span>
              )}
            </div>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-32 overflow-y-auto space-y-1 pr-1">
              {breakdown.slice(0, 6).map((game) => (
                <div
                  key={game.id}
                  className="flex items-center justify-between text-xs bg-background/40 rounded px-2 py-1.5"
                >
  ...
  return (
    <>
      <tr
        ...
        className={cn("transition-all duration-200 cursor-pointer group", getRowBackground(rank))}
      >
        {/* Rank */}
        <td className="px-3 py-3 whitespace-nowrap">
          <div className="flex items-center gap-1.5">
            <span className={cn("font-display font-bold text-lg w-5", getRankColor(rank))}>{rank}</span>
            {getRankIcon(rank)}
          </div>
        </td>

        {/* Name - Auto width based on content */}
        <td className="px-3 py-3 whitespace-nowrap">{renderName()}</td>
        ...
        <td className="px-3 py-3 text-center whitespace-nowrap">
          <div className="flex items-center justify-center">
            {qualifies ? (
              <Badge variant="default" className="bg-secondary text-secondary-foreground text-xs px-1.5 py-0.5">
  ...
  return (
    <>
      <tr onClick={handleRowClick} className={cn("transition-all duration-200 cursor-pointer", getRowBackground(rank))}>
        ...
        <td className="sticky left-0 z-10 py-2.5 bg-background w-fit whitespace-nowrap px-2">
          <div className="flex items-center gap-1">
            <span className={cn("font-display font-bold text-xs w-4 shrink-0", getRankColor(rank))}>{rank}</span>
            {renderAvatar()}
            {mode === "doubles" && players ? (
              <div className="flex flex-col justify-center leading-tight">
                <span className="font-medium text-foreground text-[10px]">{players[0]}</span>
                <span className="font-medium text-foreground/80 text-[10px]">{players[1]}</span>
              </div>
            ) : (
              <div className="flex items-center gap-0.5">
                <span className="font-medium text-foreground text-[11px] truncate max-w-[55px]">{name}</span>
                {mode === "singles" && (
                  <ChevronDown
                    className={cn(
        ...
        {/* Games Played */}
        <td className="px-2 py-2.5 text-center font-medium text-foreground text-xs tabular-nums">{gamesPlayed}</td>
