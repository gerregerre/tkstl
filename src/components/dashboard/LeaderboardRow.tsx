      ...
      >
        ...
        <td className="w-[80px] px-5 py-4">
          <div className="flex items-center gap-2.5">
            <span className={cn("font-display font-bold text-lg w-6", getRankColor(rank))}>{rank}</span>
            {getRankIcon(rank)}
          </div>
        </td>

        {/* Name - Flexible, takes remaining space */}
        <td className="px-5 py-4">{renderName()}</td>
        ...
        <td className="w-[80px] px-5 py-4 text-center font-semibold text-foreground tabular-nums">{gamesPlayed}</td>
  ...
  return (
    <>
      <tr onClick={handleRowClick} className={cn("transition-all duration-200 cursor-pointer", getRowBackground(rank))}>
        ...
        <td className="sticky left-0 z-10 px-1 py-2.5 bg-background min-w-[115px]">
          <div className="flex items-center gap-1">
            <span className={cn("font-display font-bold text-xs w-4 shrink-0", getRankColor(rank))}>{rank}</span>
            {renderAvatar()}
            {mode === "doubles" && players ? (
              <div className="flex flex-col justify-center leading-tight">
                <span className="font-medium text-foreground text-[11px]">{players[0]}</span>
                <span className="font-medium text-foreground/80 text-[11px]">{players[1]}</span>
              </div>
            ) : (
              <div className="flex items-center gap-0.5">
                <span className="font-medium text-foreground text-xs truncate max-w-[70px]">{name}</span>
                {mode === "singles" && (
                  <ChevronDown
                    className={cn(
